// Usage:
// ./main -type=<cpubound|iobound|balanced> -coroutines=<n> -threads=<m>
//
// Time this program:
// $ time ./main -type=cpubound -coroutines=5 -threads=2
...
DEFINE_string(type, "", "cpubound|iobound|balanced");
...
void ValidateFlags() {
  ...
  if ((t != "cpubound" && t != "iobound" && t != "balanced") ||
      FLAGS_coroutines <= 0 || FLAGS_threads <= 0) {
    std::puts("Invalid flags!");
    std::puts("Usage Example: ./main -type=iobound -coroutines=4 -threads=2");
    std::exit(1);
  }
}
...
int main(int argc, char* argv[]) {
  ...
  auto progress = std::make_shared<Progress>(100);
